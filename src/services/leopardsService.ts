...
export const getActivityLog = async (startDate?: string, endDate?: string, cnNumber?: string) => {
    // Refresh config from DB
    await fetchLeopardsConfig();

    // If credentials are mock, return mock logs
    if (LEOPARDS_API_KEY === "" || LEOPARDS_API_KEY === "YOUR_API_KEY") {
        return getMockActivityLog(startDate, endDate, cnNumber);
    }

    try {
        console.log(`[LEOPARDS] Fetching activity log / shipper advice list from: ${LEOPARDS_API_URL}shipperAdviceList/format/json/`);
        
        const response = await axios.post(`${LEOPARDS_API_URL}shipperAdviceList/format/json/`, {
            api_key: LEOPARDS_API_KEY,
            api_password: LEOPARDS_API_PASSWORD,
            dateFrom: startDate || "",
            toDate: endDate || "",
        ...
        return { status: 1, totalrecords: 0, data: [], message: response.data?.error || "No data found" };
    } catch (error: any) {
        console.error("Leopards Activity Log Error:", error.message);
        throw new Error(`Leopards Activity Log Failed: ${error.message}`);
    }
};

const getMockActivityLog = (startDate?: string, endDate?: string, cnNumber?: string) => {
    return {
        status: 1,
        totalrecords: 3,
        data: [
            {
                id: 101,
                cn_number: cnNumber || "701234567",
                shipper_name: "Admin Store",
                consignee_name: "Muhammad Baig",
                cour_date: "2026-03-31",
                status: "Delivered",
                reason: "Successful delivery",
                remarks: "Payment received by rider",
                shipper_advice_status: "Approved"
            },
            {
                id: 102,
                cn_number: "701234568",
                ...
                cour_date: "2026-04-01",
                status: "In Transit",
                reason: "Forwarded to destination",
            ...
            },
            {
                ...
                cour_date: "2026-04-05",
                status: "Booked",
                reason: "New shipment",
                remarks: "Pickup requested",
                shipper_advice_status: "New"
            }
        ]
    };
};
